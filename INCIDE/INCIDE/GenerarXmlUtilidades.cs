...
namespace INCIDE
{
    public partial class GenerarXmlUtilidades : Form

    {
        ...
        public GenerarXmlUtilidades(INCIDEControl pIcontrol)
        {
            InitializeComponent();
       ...
       bool Validar_CI_Repetidos(Dictionary<string, string> listadic,string CI_nuevo)
       {
        ...
        Dictionary<string, string> Cargar_Diccionario_Grid()
        {
            ...
            foreach (var item in radGridView1.Rows)
            {
                ...
                if (item.Cells[4].Value != null)
                    IMPORTE = Math.Round(Convert.ToDecimal(item.Cells[4].Value), 2).ToString();
                if (Math.Round(Convert.ToDecimal(item.Cells[4].Value), 2) > 0)
                {
                    try
                    {
        ...
        private void FUNCION_Genera_XML(Dictionary<string, string> listadic, string dirGuardar)
        {
            ...
            foreach (var dato in listadic)
            {
                ...
                fs.SaveToFile("<TiemClave>2027</TiemClave>");
